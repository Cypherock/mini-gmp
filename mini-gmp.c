...
static void *(*gmp_reallocate_func)(void *, size_t,
                                    size_t) = gmp_default_realloc;
...
#define gmp_realloc(ptr, old_size, size)                                       \
  ((*gmp_reallocate_func)(ptr, old_size, size))

static mp_ptr gmp_alloc_limbs(mp_size_t size) {
  return (mp_ptr)gmp_alloc(size * sizeof(mp_limb_t));
}

static mp_ptr gmp_realloc_limbs(mp_ptr old, mp_size_t old_size,
                                mp_size_t size) {
  assert(size > 0);
  return (mp_ptr)gmp_realloc(old, old_size * sizeof(mp_limb_t),
...
mp_bitcnt_t mpn_scan1(mp_srcptr ptr, mp_bitcnt_t bit) {
  ...
  return mpn_common_scan(ptr[i] & (GMP_LIMB_MAX << (bit % GMP_LIMB_BITS)), i,
                         ptr, i, 0);
}
...
static void mpn_div_qr_2_preinv(mp_ptr qp, mp_ptr np, mp_size_t nn,
                                const struct gmp_div_inverse *inv) {
  unsigned shift;
  mp_size_t i;
...
static void mpn_div_qr_pi1(mp_ptr qp, mp_ptr np, mp_size_t nn, mp_limb_t n1,
  ...
  do {
    ...
    if (n1 == d1 && n0 == d0) {
      ...
      n1 =
          np[dn - 1 + i]; /* update n1, last loop's value will now be invalid */
    } else {
...
static void mpn_div_qr_preinv(mp_ptr qp, mp_ptr np, mp_size_t nn, mp_srcptr dp,
                              mp_size_t dn, const struct gmp_div_inverse *inv) {
  assert(dn > 0);
  assert(nn >= dn);
...
static size_t mpn_get_str_bits(unsigned char *sp, unsigned bits, mp_srcptr up,
                               mp_size_t un) {
  unsigned char mask;
  size_t sn, j;
  mp_size_t i;
...
static size_t mpn_get_str_other(unsigned char *sp, int base,
                                const struct mpn_base_info *info, mp_ptr up,
                                mp_size_t un) {
...
static mp_size_t mpn_set_str_bits(mp_ptr rp, const unsigned char *sp, size_t sn,
                                  unsigned bits) {
  mp_size_t rn;
  mp_limb_t limb;
  unsigned shift;
...
static mp_size_t mpn_set_str_other(mp_ptr rp, const unsigned char *sp,
...
int mpz_cmpabs(const mpz_t u, const mpz_t v) {
  return mpn_cmp4(u->_mp_d, GMP_ABS(u->_mp_size), v->_mp_d,
                  GMP_ABS(v->_mp_size));
}
...
static int mpz_div_qr(mpz_t q, mpz_t r, const mpz_t n, const mpz_t d,
...
static void mpz_div_q_2exp(mpz_t q, const mpz_t u, mp_bitcnt_t bit_index,
                           enum mpz_div_round_mode mode) {
  mp_size_t un, qn;
  mp_size_t limb_cnt;
  mp_ptr qp;
...
static void mpz_div_r_2exp(mpz_t r, const mpz_t u, mp_bitcnt_t bit_index,
                           enum mpz_div_round_mode mode) {
  mp_size_t us, un, rn;
  mp_ptr rp;
  mp_limb_t mask;
...
static unsigned long mpz_div_qr_ui(mpz_t q, mpz_t r, const mpz_t n,
                                   unsigned long d,
                                   enum mpz_div_round_mode mode) {
  unsigned long ret;
  mpz_t rr, dd;
...
static int gmp_lucas_mod(mpz_t V, mpz_t Qk, long Q, mp_bitcnt_t b0,
                         const mpz_t n) {
  mp_bitcnt_t bs;
  mpz_t U;
  int res;
...
static int gmp_millerrabin(const mpz_t n, const mpz_t nm1, mpz_t y,
...
static void mpz_abs_sub_bit(mpz_t d, mp_bitcnt_t bit_index) {
  ...
  gmp_assert_nocarry(
      mpn_sub_1(dp + limb_index, dp + limb_index, dn - limb_index, bit));
